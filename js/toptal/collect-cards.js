...
*/
var CollectCard = /** @class */ (function () {
    function CollectCard() {
    }
    CollectCard.collectCards = function (A) {
        A = A.map(function (i) { return i =
            i.replace('A', '1')
                .replace('T', '10')
                .replace('J', '11')
                .replace('Q', '12')
                ...
                .replace('H', '2')
                .replace('D', '3'); });
        var decks = Array.apply(null, { length: 4 })
            .map(function () { return Array.apply(null, { length: 13 }).map(function () { return 0; }); });
        A.forEach(function (e) {
            var type = Number(e.charAt(e.length - 1));
            var value = Number(e.toString().substr(0, e.length - 1));
            decks[type][value - 1] = decks[type][value - 1] + 1;
        });
        return Math.min.apply(null, decks.map(function (i) { return Math.min.apply(null, i); }));
    };
    return CollectCard;
}());
var CollectCardTest = /** @class */ (function () {
    function CollectCardTest() {
    ...
    CollectCardTest.prototype.exec = function (testNo, cards) {
        var result = CollectCard.collectCards(cards);
        console.log('');
    ...
    return CollectCardTest;
}());
new CollectCardTest();
//# sourceMappingURL=collect-cards.js.map